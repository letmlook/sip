...
use siprs_core::{Host, ParseError, TransportProtocol};
...
pub struct ViaHeader {
    ...
    pub version: siprs_core::SipVersion,
    /// 传输协议
...
impl FromStr for ViaHeader {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // 验证 SIP 版本号
        if version_str != siprs_core::SipVersion::VERSION {
...
mod tests {
    ...
    fn test_via_header_new() {
        ...
        assert_eq!(via.version, siprs_core::SipVersion);
    ...
    fn test_via_header_version_is_zst() {
        // 验证 version 字段为零大小类型
        assert_eq!(std::mem::size_of::<siprs_core::SipVersion>(), 0);
        let via = ViaHeader::new(
            TransportProtocol::Udp,
            Host::Domain("example.com".to_string()),
            Some(5060),
