...
use siprs_core::{RegistrationError, SipVersion, StatusCode};
use siprs_message::{
...
impl Registrar {
    ...
    fn build_200_ok_response(&self, aor: &str, contact_uri: &str, expires: u64) -> SipResponse {
        // 构建 Contact 头部
        let contact_sip_uri = match siprs_message::SipUri::parse(contact_uri) {
            Ok(uri) => uri,
            Err(_) => {
        ...
        // 构建 To 头部（添加 tag）
        let to_uri = match siprs_message::SipUri::parse(aor) {
            Ok(uri) => uri,
            Err(_) => {
                return self
...
mod tests {
    ...
    use siprs_message::{CSeqHeader, CallId, RequestLine, SipUri, Tag};
