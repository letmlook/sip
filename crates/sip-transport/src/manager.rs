...
use sip_core::config::{TlsConfig, TransportConfig};
use sip_core::metrics::SipMetrics;
use sip_core::{Host, TransportError, TransportProtocol};
use sip_message::builder::MessageBuilder;
use sip_message::headers::{HeaderName, HeaderValue, ViaHeader};
...
impl TransportManager {
    ...
    pub async fn send(&self, message: &SipMessage, target_uri: &str) -> Result<(), TransportError> {
        // 1. 解析目标 URI
        let uri =
            sip_message::SipUri::parse(target_uri).map_err(|e| TransportError::SendFailed {
    ...
    /// - 默认 → UDP
    fn determine_transport(&self, uri: &sip_message::SipUri) -> TransportProtocol {
        // sips: URI 强制使用 TLS
        if uri.scheme == sip_message::UriScheme::Sips {
...
mod tests {
    ...
    fn test_determine_transport_sips() {
        let manager = create_test_manager();
        let uri = sip_message::SipUri::parse("sips:bob@example.com").unwrap();
    ...
    fn test_determine_transport_tcp_param() {
        let manager = create_test_manager();
        let uri = sip_message::SipUri::parse("sip:bob@example.com;transport=tcp").unwrap();
    ...
    fn test_determine_transport_udp_param() {
        let manager = create_test_manager();
        let uri = sip_message::SipUri::parse("sip:bob@example.com;transport=udp").unwrap();
    ...
    fn test_determine_transport_default() {
        let manager = create_test_manager();
        let uri = sip_message::SipUri::parse("sip:bob@example.com").unwrap();
        assert_eq!(manager.determine_transport(&uri), TransportProtocol::Udp);
    }
    ...
    fn test_add_via_header() {
        ...
        // 创建一个没有 Via 头部的消息
        let uri = sip_message::SipUri::parse("sip:bob@example.com").unwrap();
        let mut headers = sip_message::HeaderCollection::new();
        headers.insert(
            HeaderName::CallId,
            HeaderValue::CallId(sip_message::CallId("test@example.com".to_string())),
        );
        headers.insert(
            HeaderName::CSeq,
            HeaderValue::CSeq(sip_message::CSeqHeader::new(1, sip_message::Method::Invite)),
        );

        let mut message = SipMessage::Request(sip_message::SipRequest {
