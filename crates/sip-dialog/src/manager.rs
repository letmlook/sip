...
use sip_core::{metrics::SipMetrics, CSeqNumber, DialogError, Tag};
...
impl DialogManager {
    ...
    pub async fn create_uac_dialog(
        ...
        if status_code.is_success() {
            ...
            let from_tag = response
                .headers
                .get(&sip_message::HeaderName::From)
                ...
                if let Some(existing_dialog) = dialogs.get_mut(&dialog_id) {
                    ...
                    if existing_dialog.state == DialogState::Early {
                        ...
                        if let Some(contact_uri) = response
                            .headers
                            .get(&sip_message::HeaderName::Contact)
    ...
    pub async fn handle_invite_2xx(
        ...
        let invite_cseq = invite
            .headers
            .get(&sip_message::HeaderName::CSeq)
...
mod tests {
    ...
    fn create_test_invite() -> SipRequest {
        ...
        headers.insert(
            HeaderName::CallId,
            HeaderValue::CallId(sip_message::CallId("test-call-id@example.com".to_string())),
        );
        headers.insert(
            HeaderName::CSeq,
