...
use sip_message::{BranchId, Method, SipRequest, SipResponse};
...
impl TransactionKey {
    ...
    pub fn from_request(request: &SipRequest) -> Option<Self> {
        let via_value = request.headers.get(&sip_message::HeaderName::Via)?;
    ...
    pub fn from_response(response: &SipResponse) -> Option<Self> {
        let via_value = response.headers.get(&sip_message::HeaderName::Via)?;
        let via = via_value.as_via()?;

        let cseq_value = response.headers.get(&sip_message::HeaderName::CSeq)?;
...
pub enum TransactionAction {
    /// 发送 SIP 消息
    SendMessage {
        message: sip_message::SipMessage,
        addr: SocketAddr,
