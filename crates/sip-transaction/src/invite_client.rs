    ...
    fn handle_response_calling(
        ...
        if status_code.is_provisional() {
            ...
            actions.push(TransactionAction::SendMessage {
                message: sip_message::SipMessage::Request(ack),
    ...
    fn build_ack(&self, response: &SipResponse) -> SipRequest {
        ...
        if let Some(cseq_val) = original.headers.get(&HeaderName::CSeq) {
            if let Some(cseq) = cseq_val.as_cseq() {
                headers.insert(
                    HeaderName::CSeq,
                    HeaderValue::CSeq(sip_message::CSeqHeader::new(cseq.sequence.0, Method::Ack)),
                );
            }
        }
...
mod tests {
    ...
    use sip_message::{CSeqHeader, CallId, StatusLine, ViaHeader};
    ...
    fn test_calling_to_completed_on_3xx_6xx() {
        ...
        let has_ack = actions.iter().any(|a| matches!(
            a,
            TransactionAction::SendMessage { message, .. } if matches!(message, sip_message::SipMessage::Request(req) if req.request_line.method == Method::Ack)
        ));
        assert!(has_ack);
    }
    ...
    fn test_timer_a_retransmits_invite() {
        ...
        let has_invite = actions.iter().any(|a| matches!(
            a,
            TransactionAction::SendMessage { message, .. } if matches!(message, sip_message::SipMessage::Request(req) if req.request_line.method == Method::Invite)
        ));
        assert!(has_invite);
    }
