...
use sip_core::{SipError, TransportProtocol};
use sip_dialog::DialogManager;
use sip_message::{Method, SipMessage, SipRequest, SipResponse};
use sip_registration::{RegistrationId, RegistrationManager};
use sip_transaction::{TransactionEvent, TransactionId, TransactionManager};
use sip_transport::{TransportEvent, TransportManager};
...
pub struct SipEngine {
    ...
    dialog_event_rx: Option<mpsc::UnboundedReceiver<sip_dialog::DialogEvent>>,
    /// 注册事件接收端（将在 start_event_loop 中消费）
    reg_event_rx: Option<mpsc::UnboundedReceiver<sip_registration::RegistrationEvent>>,
}
...
impl SipEngine {
    ...
    pub async fn start(&mut self) -> Result<(), SipError> {
        ...
        let bind_addr: SocketAddr = format!("{}:{}", self.config.bind_ip, self.config.sip_port)
            ...
            .map_err(|e: std::net::AddrParseError| {
                SipError::Config(sip_core::ConfigError::InvalidValue {
                    ...
                    match event {
                        TransactionEvent::Timeout { transaction_id } => {
                            ...
                            let _ = event_tx_clone.send(SipEvent::Error(SipError::Transaction(
                                sip_core::TransactionError::Timeout {
    ...
    async fn handle_incoming_request(
        ...
        let call_id = request
            .headers
            .get(&sip_message::HeaderName::CallId)
