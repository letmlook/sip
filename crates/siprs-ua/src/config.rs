...
pub fn build_invite_request(
...
) -> Result<(siprs_message::SipRequest, CallId), String> {
    // 解析目标 URI
    ...
    // 构建请求
    let request = siprs_message::SipRequest {
...
pub fn build_response(
    request: &siprs_message::SipRequest,
    status_code: u16,
    reason_phrase: &str,
    ...
    let status = siprs_core::StatusCode(status_code);
