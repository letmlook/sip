...
//!
//! ```ignore
//! use siprs_ua::gb28181_server::{Gb28181ServerConfig, Gb28181Server};
//!
...
struct Notify {
    /// 命令类型
    cmd_type: siprs_gb28181_xml::CmdType,
    /// 命令序号
    sn: u32,
    /// 目标设备编码
    device_id: String,
    /// 报警列表
    alarm_list: Vec<siprs_gb28181_xml::AlarmInfo>,
    /// 移动位置列表
    mobile_position_list: Vec<siprs_gb28181_xml::MobilePositionInfo>,
}

impl Notify {
    /// 从 XML 字符串解析 Notify 消息
    fn from_xml(xml: &str) -> Result<Self, siprs_gb28181_xml::XmlError> {
        // 验证根元素
        ...
        let alarm_list = if let Some(list_inner) = extract_tag_with_attrs_local(&inner, "AlarmList")
        {
            ...
            for item_content in &item_contents {
                if let Ok(item) = siprs_gb28181_xml::AlarmInfo::from_xml_item(item_content) {
                    items.push(item);
                }
            }
            items
...
pub enum Gb28181ServerEvent {
    ...
    CatalogResponse {
        device_id: String,
        devices: Vec<siprs_gb28181_xml::DeviceItem>,
    },
    /// 收到设备信息响应
    DeviceInfoResponse {
        device_id: String,
    ...
    RecordQueryResponse {
        device_id: String,
        records: Vec<siprs_gb28181_xml::RecordItem>,
    },
    /// 收到设备状态响应
    ...
    AlarmReceived {
        device_id: String,
        alarms: Vec<siprs_gb28181_xml::AlarmInfo>,
    },
    /// 收到移动位置通知
    MobilePositionReceived {
        device_id: String,
        positions: Vec<siprs_gb28181_xml::MobilePositionInfo>,
    },
    /// 视频点播 INVITE 已发送
...
impl Gb28181Server {
    ...
    pub fn new(config: Gb28181ServerConfig) -> Self {
        ...
        let auth_handler = Arc::new(siprs_registration::auth::DigestAuthHandler::new());
    ...
    ///
    /// 返回应发送给设备的 SIP 响应。
    pub async fn handle_register(&self, request: &SipRequest) -> siprs_message::SipResponse {
        // 通过 Registrar 处理注册
    ...
    pub async fn handle_alarm(&self, request: &SipRequest) -> Result<(), String> {
        ...
        let alarms = if let Ok(notify) = Notify::from_xml(&body) {
            if notify.cmd_type == siprs_gb28181_xml::CmdType::Alarm {
                notify.alarm_list
            } else {
                Vec::new()
            }
        } else if let Ok(msg) = siprs_gb28181_xml::parse_xml(&body) {
            match msg {
                siprs_gb28181_xml::Message::Response(resp)
                    if resp.cmd_type == siprs_gb28181_xml::CmdType::Alarm =>
                {
                    resp.alarm_list
                }
                _ => Vec::new(),
    ...
    pub async fn handle_mobile_position(&self, request: &SipRequest) -> Result<(), String> {
        ...
        let positions = if let Ok(notify) = Notify::from_xml(&body) {
            if notify.cmd_type == siprs_gb28181_xml::CmdType::MobilePositionNotify {
            ...
            match msg {
                siprs_gb28181_xml::Message::Response(resp)
                    if resp.cmd_type == siprs_gb28181_xml::CmdType::MobilePositionNotify =>
                {
    ...
    pub async fn handle_message_response(&self, request: &SipRequest) -> Result<(), String> {
        ...
        match msg {
            siprs_gb28181_xml::Message::Response(response) => {
                match response.cmd_type {
                    ...
                    siprs_gb28181_xml::CmdType::DeviceInfo => {
                        // 提取设备信息
                        let name = response
                            .device_list
                            .first()
            ...
            siprs_gb28181_xml::Message::Control(_) => {
        ...
        if siprs_gb28181_xml::parse_xml(&body).is_err() {
    ...
    pub async fn query_device_info(&self, device_id: &str) -> Result<(), String> {
        ...
        let query = siprs_gb28181_xml::Query::device_info(sn, device_id_obj);
        let xml = query.to_xml();
    ...
    pub async fn query_record(
        ...
        let query = siprs_gb28181_xml::Query::record_query(sn, device_id_obj, start, end);
        let xml = query.to_xml();
    ...
    pub async fn query_device_status(&self, device_id: &str) -> Result<(), String> {
        ...
        let query = siprs_gb28181_xml::Query::device_status(sn, device_id_obj);
        let xml = query.to_xml();
    ...
    pub async fn ptz_control(
        &self,
        device_id: &str,
        direction: siprs_gb28181_xml::PtzDirection,
        speed: u8,
    ) -> Result<(), String> {
        let sn = self.next_sn().await;
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id)
        ...
        let ptz_speed = siprs_gb28181_xml::PtzSpeed::new(speed);
        let control = siprs_gb28181_xml::Control::ptz_with_direction(
            sn,
            device_id_obj,
            direction,
            ptz_speed,
    ...
    pub async fn remote_start(&self, device_id: &str) -> Result<(), String> {
        ...
        let control = siprs_gb28181_xml::Control::remote_start(sn, device_id_obj);
        let xml = control.to_xml();
    ...
    pub async fn drag_playback(&self, device_id: &str, play_time: &str) -> Result<(), String> {
        ...
        let control = siprs_gb28181_xml::Control::drag_playback(sn, device_id_obj, play_time);
        let xml = control.to_xml();
...
pub fn build_server_message_request(
    ...
    // 消息体
    let body = siprs_message::Body::new("Application/MANSCDP+xml", xml_body.as_bytes().to_vec());
...
mod tests {
    ...
    fn test_build_server_message_request_catalog_query() {
        ...
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id).unwrap();
        let query = siprs_gb28181_xml::Query::catalog(1, device_id_obj);
        let xml = query.to_xml();
    ...
    fn test_build_server_message_request_ptz_control() {
        let device_id = "34020000001320000001";
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id).unwrap();
        let control = siprs_gb28181_xml::Control::ptz(3, device_id_obj, "A5201F1F00000085");
        let xml = control.to_xml();
    ...
    fn test_build_server_message_request_remote_start() {
        let device_id = "34020000001320000001";
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id).unwrap();
        let control = siprs_gb28181_xml::Control::remote_start(10, device_id_obj);
        let xml = control.to_xml();
    ...
    fn test_build_server_message_request_record_query() {
        let device_id = "34020000001320000001";
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id).unwrap();
        let query = siprs_gb28181_xml::Query::record_query(
            10,
            device_id_obj,
            "2024-01-01T00:00:00",
            "2024-01-31T23:59:59",
    ...
    fn test_build_server_message_request_device_status() {
        let device_id = "34020000001320000001";
        let device_id_obj = siprs_gb28181_codec::DeviceId::parse(device_id).unwrap();
        let query = siprs_gb28181_xml::Query::device_status(5, device_id_obj);
        let xml = query.to_xml();
