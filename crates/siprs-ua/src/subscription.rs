...
//!
//! ```ignore
//! use siprs_ua::subscription::{SubscriptionManager, build_catalog_subscribe};
//!
...
use siprs_core::{Host, SipVersion};
use siprs_message::{
...
///
/// ```ignore
/// use siprs_ua::subscription::SubscriptionManager;
///
/// let (manager, event_rx) = SubscriptionManager::new();
///
/// // 发起订阅
...
fn build_subscribe_request_with_server(
    ...
    let via = ViaHeader::new(siprs_core::TransportProtocol::Udp, via_host, Some(5060));
...
pub fn build_subscribe_with_config(
    ...
    // 构建 Contact 头部
    let contact_header = siprs_message::ContactHeader::new(contact_uri);
...
fn build_device_uri(device_id: &str, server_addr: &str) -> SipUri {
    ...
    SipUri {
        scheme: siprs_message::UriScheme::Sip,
        user_info: Some(siprs_message::UserInfo {
            user: device_id.to_string(),
            password: None,
        }),
        host,
        port: None,
        params: siprs_message::UriParams::new(),
...
mod tests {
    ...
    async fn test_handle_subscribe_response_423() {
        ...
        // 构建 423 响应
        let mut response = siprs_message::SipResponse {
            status_line: siprs_message::StatusLine {
                version: SipVersion,
                status_code: siprs_core::StatusCode(423),
    ...
    async fn test_handle_subscribe_response_error() {
        ...
        // 构建 404 响应
        let response = siprs_message::SipResponse {
            status_line: siprs_message::StatusLine {
                version: SipVersion,
                status_code: siprs_core::StatusCode(404),
    ...
    fn test_build_catalog_subscribe() {
        ...
        assert_eq!(uri.scheme, siprs_message::UriScheme::Sip);
    ...
    fn build_200_ok_response(request: &SipRequest, expires: u32) -> SipResponse {
        ...
        SipResponse {
            status_line: siprs_message::StatusLine {
                version: SipVersion,
                status_code: siprs_core::StatusCode(200),
